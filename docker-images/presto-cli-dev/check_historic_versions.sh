...
for orig_version in latest $versions_to_check; do
    ...
    found_version=$(docker run --rm -ti "khulnasoft/presto-cli-dev:$orig_version" --version | awk '{print $3}' | tr -d '\r')
    echo
    if [ "$found_version" = "$version" ]; then
        echo "VALIDATED: khulnasoft/presto-cli-dev:$orig_version contains:  $found_version"
    else
        fail=1
        echo "INVALID: khulnasoft/presto-cli-dev:$orig_version contains wrong version of presto, found:  $found_version !!!"
    fi
    echo
    if [ $count -gt 1 ]; then
        docker rmi "khulnasoft/presto-cli-dev:$orig_version"
    fi
    echo
done
