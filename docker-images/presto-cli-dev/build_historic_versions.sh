...
for version in $versions_to_build; do
    ...
    docker build -t "khulnasoft/presto-cli-dev:$version" --build-arg PRESTO_DEVELOPMENT_VERSION="$version" $no_cache .
    [ -n "${NOPUSH:-}" ] || docker push "khulnasoft/presto-cli-dev:$version"
    ...
    if [ $count -gt 1 ]; then
        docker rmi "khulnasoft/presto-cli-dev:$version"
    fi
    echo
done
