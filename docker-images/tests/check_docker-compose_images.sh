...
for directory in *; do
    ...
    for compose_file in "$directory/"*docker-compose*.yml; do
        ...
        if grep -q image "$compose_file"; then
            ...
            if ! grep -Eq \
                 -e "^[[:space:]]*image:[[:space:]]*khulnasoft/$directory(:\\$\\{VERSION:-latest\\}|:latest)?[[:space:]]*$" \
                 -e "^[[:space:]]*image:[[:space:]]*khulnasoft/${image_name_alternate}(:\\$\\{VERSION:-$image_tag_alternate\\}|:$image_tag_alternate)?[[:space:]]*$" "$compose_file"; then
