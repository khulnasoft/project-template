...
for version in $versions_to_build; do
    ...
    [ -z "${PULL:-}" ] || docker pull "khulnasoft/presto-dev:$version"
    docker build -t "khulnasoft/presto-dev:$version" --build-arg PRESTO_VERSION="$version" $no_cache .
    [ -n "${NOPUSH:-}" ] || docker push "khulnasoft/presto-dev:$version"
    ...
    if [ $count -gt 1 ] && [ -z "${NODELETE:-}" ]; then
        docker rmi "khulnasoft/presto-dev:$version"
    fi
    echo
done
