...
export TRAVIS_USER=KhulnaSoft # put this in .bashrc
travis_last_log.py /DevOps-Python-tools   # shorter to type than full 'KhulnaSoft/DevX/tree/main/tools/python'

As a convenience you may supply either job id or repo as an argument without any switch and it'll infer it as a repo if
if contains a slash but no url (eg. KhulnaSoft/nagios-plugins) otherwise it'll assume it's a job id, strip any leading
...
try:
    ...
    from khulnasoft.utils import prog, log, support_msg_api, jsonpp, qquit, isInt, isStr, isJson
    from khulnasoft.utils import UnknownError, code_error, strip_ansi_escape_codes
    from khulnasoft.utils import validate_chars, validate_alnum, validate_int
    from khulnasoft import CLI
    from khulnasoft import RequestHandler
...
class TravisLastBuildLog(CLI):
    ...
    def process_options(self):
        ...
        if self.job_id:
            ...
            # https://travis-ci.org/KhulnaSoft/nagios-plugins/jobs/283840596#L1079
        ...
        if not json_data or \
           ...
           not json_data['builds']:
            qquit('UNKNOWN', "no Travis CI builds returned by the Travis API."
                  ...
                  + " Also remember the repo is case sensitive, for example 'khulnasoft/nagios-plugins' returns this"
                  + " blank build set whereas 'KhulnaSoft/Nagios-Plugins' succeeds"
