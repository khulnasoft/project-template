...
sed_script="
s|https\\?://www.linkedin.com/in/KhulnaSoft|https://www.linkedin.com/in/KhulnaSoft|gi;
s|https\\?://ghcr.io/khulnasoft/|https://ghcr.io/KhulnaSoft/|gi;

$(
    sed 's/#.*//; /^[[:space:]]*$/d; s/:/ /' "$srcdir/../setup/repos.txt" |
    while read -r repo shortname; do
        if [ -n "$repo" ]; then
            echo "s|https\\?://github.com/khulnasoft/$repo|https://github.com/KhulnaSoft/$repo|gi;"
        fi
        if [ -n "$shortname" ]; then
            echo "s|https\\?://github.com/khulnasoft/$shortname$|https://github.com/KhulnaSoft/$repo|gi;"
            echo "s|https\\?://github.com/khulnasoft/$shortname/|https://github.com/KhulnaSoft/$repo/|gi;"
        fi
        echo
    done
