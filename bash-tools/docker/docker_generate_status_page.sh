...
#
#   DOCKER_USER=khulnasoft ./github_generate_status_docker.sh
#
...
#
#   DOCKER_USER=khulnasoft ./github_generate_status_page.sh  khulnasoft/hbase khulnasoft/zookeeper khulnasoft/nagios-plugins ...
#
#   DOCKER_USER=khulnasoft ./github_generate_status_page.sh  hbase zookeeper nagios-plugins ...
...
if [ -z "$repolist" ]; then
    repolist="$(dockerhub_search.py -n 100 khulnasoft | awk '/^khulnasoft\/.*[[:space:]]\[OK]/{print $1}' | sort)"
fi
...
generated by \`${0##*/}\` in [KhulnaSoft/DevOps-Bash-tools](https://github.com/khulnasoft/project-template/tree/main/scripts)
