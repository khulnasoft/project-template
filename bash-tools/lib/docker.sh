...
dockerhub_latest_version(){
    ...
    set +e
    local version
    version="$(curl -sS "https://raw.githubusercontent.com/KhulnaSoft/DevX/tree/main/docker-images/master/$repo/Dockerfile" | awk -F= '/^ARG[[:space:]]+[A-Za-z0-9_]+_VERSION=/ {print $2; exit}')"
    set -e
    if [ -z "$version" ]; then
        version='.*'
    fi
