...
{owner} - repo owner eg. KhulnaSoft
...
if [ -n "$repolist" ]; then
    ...
    repolist="$(curl -sSL https://raw.githubusercontent.com/KhulnaSoft/bash-tools/master/setup/repos.txt | sed 's/#.*//')"
fi
