...
docker_get_images(){
    ...
    #echo "$(dockerhub_search.py khulnasoft -n 1000 | tail -n +2 | awk '{print $1}' | sort) $(sed 's/#.*//;/^[[:space:]]*$/d' "$bash_tools/setup/docker-images.txt" | uniq_order_preserved.pl)"
...
dockerpullgithub(){
    dockerpull1 khulnasoft/{nagios-plugins,pytools,tools,centos-github,debian-github,ubuntu-github,alpine-github}
}
