...
"nmap <silent> ;u :w<CR> :! bash -c 'grep -vi khulnasoft "%" ; grep -i khulnasoft "%" \| tail -r' \| urlview <CR> :<CR>
nmap <silent> ;u :w<CR> :! bash -c '{ urlextract.sh "%" ; terraform_registry_url_extract.sh "%"; } \| terraform_registry_url_to_https.sh \| lines_to_end.sh khulnasoft \| urlview' <CR> :<CR>
...
"    autocmd FileType vim              let b:comment_char = '"'
    ...
    " https://github.com/KhulnaSoft/Python-DevOps-Tools repo which should be downloaded, run 'make' and add to $PATH
    ...
    " these tools are in the https://github.com/KhulnaSoft/DevX/tree/main/tools/python & DevOps-Bash-tools repos which should be downloaded, run 'make' and add to $PATH
