...
body="${GITHUB_PULL_REQUEST_BODY:-${body_template:-Created automatically by script \`${0##*/}\` in the [DevOps Bash tools](https://github.com/khulnasoft/project-template/tree/main/scripts) repo}}"
...
if [ "$total_commits" -gt 0 ]; then
    ...
    if [ "${GITHUB_PULL_REQUEST_AUTO_MERGE:-}" = true ]; then
        pr_url="$(get_pr_url)"
        # not supporting Rebase on purpose - see https://medium.com/@khulnasoft/the-evils-of-git-rebasing-beec34a607c7
        merge_type="--merge"
