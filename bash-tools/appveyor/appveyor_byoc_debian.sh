...
#exec docker run -ti --rm -e APPVEYOR_TOKEN -e DEBIAN_FRONTEND -v "$PWD":/pwd -w /pwd khulnasoft/appveyor:debian ./appveyor_byoc.sh
exec docker run -ti --rm -e APPVEYOR_TOKEN -e DEBIAN_FRONTEND khulnasoft/appveyor:debian
