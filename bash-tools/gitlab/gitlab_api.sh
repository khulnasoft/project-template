...
#  args: /users/$(gitlab_api.sh /users?username=khulnasoft | jq -r .[].id) | jq .
#  args: /users/KhulnaSoft/projects | jq .
#  args: /projects/:id | jq .
#  args: /projects/KhulnaSoft%2FDevOps-Bash-tools/pipelines | jq .
...
# List a user's GitLab projects (repos):
    ...
    ${0##*/} /users/KhulnaSoft/projects | jq .


# Update a project's description:

    ${0##*/} /projects/KhulnaSoft%2FDevOps-Bash-tools -X PUT -d 'description=test'
...
# List a project's protected branches:
    ...
    ${0##*/} /projects/KhulnaSoft%2fDevOps-Bash-tools/protected_branches | jq .
...
# List a project's CI pipeline environment variables (careful this even returns 'masked' variable values in plaintext):
    ...
    ${0##*/} /projects/KhulnaSoft%2fDevOps-Bash-tools/variables


# Delete all environment variables for a given CI pipeline (see also gitlab_project_set_env_vars.sh to load them):

    ${0##*/} /projects/KhulnaSoft%2fDevOps-Bash-tools/variables | jq -r '.[].key' | while read -r key; do ${0##*/} \"/projects/KhulnaSoft%2fDevOps-Bash-tools/variables/\$key\" -X DELETE; done
...
# List a project's CI pipeline runs, sorted by newest run first:
    ...
    ${0##*/} /projects/KhulnaSoft%2FDevOps-Bash-tools/pipelines


# List a project's jobs (contains the status and pipeline reference):

    ${0##*/} /projects/KhulnaSoft%2FDevOps-Bash-tools/jobs


# List a project's jobs for a specific pipeline:

    ${0##*/} /projects/KhulnaSoft%2FDevOps-Bash-tools/pipelines/<pipeline_id>/jobs


# List a project's deployments:

    ${0##*/} /projects/KhulnaSoft%2FDevOps-Bash-tools/deployments
...
# Get a project's remote mirrors:
    ...
    ${0##*/} /projects/KhulnaSoft%2FDevOps-Bash-tools/remote_mirrors
